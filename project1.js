...
server.route({
	...
	handler:(request,reply)=>{
		...
		return new Promise((resolve,reject)=>{
			...
			connection.query(`delete from beat where producer_id = ${id}`,function(error,producer,feilds){
				if(error) reject(error);
...
server.route({
	...
	handler:(request,reply)=>{
		...
		return new Promise((resolve,reject)=>{
			...
			connection.connect();
			var sql=`SELECT * from producer inner join beat on producer.producerId=beat.producer_id where approvalDate is not null and postDate_time is not null and approved=1 and producerId=${id}`;
...
server.route({
	method:"GET",
	path:"/api/producer/submitBeats/{id}",
	handler:(request,reply)=>{
		var id=request.params.id;
		var data=request.payload;
		return new Promise((resolve,reject)=>{
			...
			connection.connect();
			var sql=`SELECT * from producer inner join beat on producer.producerId=beat.producer_id where  producerId=${id} and postDate_time is  null`;
			connection.query(sql,function(error,beat,feilds){
				if(error) reject(error);
				resolve(beat);
			});
